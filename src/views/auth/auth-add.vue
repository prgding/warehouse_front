<template>
  <!-- 添加权限对话框 -->
  <el-dialog v-model="visible" destroy-on-close title="添加权限" width="25%" @close="close">
    <el-form ref="authAddForm" :model="authAdd" :rules="rules" label-position="top">
      ...
      <el-form-item label="分类">
        <el-select v-model="type">
          <el-option :disabled="authAdd.authGrade>1" :selected="authAdd.authGrade==1" label="模块" value="1"/>
          <el-option :disabled="authAdd.authGrade!=2" :selected="authAdd.authGrade==2" label="列表" value="2"/>
          <el-option :disabled="authAdd.authGrade<3" :selected="authAdd.authGrade==3" label="按钮" value="3"/>
        </el-select>
      </el-form-item>
    </el-form>
    <template #footer>
